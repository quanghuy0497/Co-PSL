...
# -----------------------------------------------------------------------------
ins_list = ['DTLZ2', 'VLMOP2', 'F2', 'RE33', 'RE36', 'RE37']
...
for test_ins in ins_list:
    ...
    x_init = np.load(f"warmup_weight/DGEMO_{test_ins}_X_{n_dim}_220.npy")    
    y_init = np.load(f"warmup_weight/DGEMO_{test_ins}_Y_{n_dim}_220.npy")
    ...
    for i_iter in range(n_iter):
        ...
        for t_step in range(n_steps):
            ...
            alpha = np.ones(n_obj)
            pref = np.random.dirichlet(alpha, n_region * n_region_vec)
        ...
        alpha = np.ones(n_obj)
        pref = np.random.dirichlet(alpha, n_region * n_candidate_per_region)
        pref_vec  = torch.tensor(pref).to(device).float() + 0.0001
